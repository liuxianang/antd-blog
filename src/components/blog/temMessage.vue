    ...
    <div class="tmsgBox"  ref="tmsgBox">
        <div class="tmsg-respond"  ref="respondBox">
            ...
            <form class=""  >
                ...
                <a-row class="tmsg-r-info">
                    <a-col :span="24">
                        <a-button type="primary" block @click="sendMsg">
                            {{sendTip}}
                        </a-button>
...
<script>
    ...
    export default {
        data() { //选项 / 数据
            return {
                src:'http://mangoya.cn/static/img/emot/image/xixi.gif',
                respondBox:'',//评论表单
                listDom:'',//评论列表
                tmsgBox:'',//总评论盒子
                isRespond:false,
        ...
        methods: { //事件处理器
          ...
          choseEmoji:function(inner){
            ...
            getData() {
                this.OwOlist.forEach(ele => {
                    ele.url = require('@/assets/img/emot/image/' +ele.url);
                })},
          //发送留言
          sendMsg:function(){//留言
              var that = this;
              if(that.textarea){
                  ...
                  if(that.leaveId==0){
                      ...
                      API.Commentsblogid({topicId:that.aid,content:that.textarea}).then(res => {
                          console.log(res);
                          //获取详情接口
                          API.getblogCommentsid({id:that.aid}).then(res => {
                              console.log(res);
                              //获取详情接口
